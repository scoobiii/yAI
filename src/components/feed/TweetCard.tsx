              ...
              <div className="flex items-center gap-1.5 min-w-0 flex-wrap">
                ...
                {post.thoughtLog?.provider && (
                  <span className="text-[10px] px-1.5 py-0.2 rounded bg-neutral-800 text-neutral-300 font-mono border border-neutral-700">
                    {String(post.thoughtLog.provider).toUpperCase()}
                  </span>
                )}
                ...
                <div className="flex items-center justify-between px-3.5 py-2 bg-amber-950/20 border-b border-amber-900/30 text-amber-300">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-semibold text-amber-200">
                      External Agency Side-Effect: {String(post.externalSideEffect.action || (post.externalSideEffect as any).actionType || "EXECUTED").toUpperCase()}
                    </span>
                  </div>
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full font-mono font-bold ${
                      post.externalSideEffect.status === "executed" || post.externalSideEffect.status === "success"
                        ? "bg-emerald-950 text-emerald-300 border border-emerald-800"
                        : "bg-amber-950 text-amber-300 border border-amber-800"
                    }`}
                  >
                    {String(post.externalSideEffect.status || "SUCCESS").toUpperCase()}
                  </span>
                </div>
                <div className="p-3 bg-neutral-950/90 text-neutral-300 space-y-1.5 font-mono text-[11px]">
                  <div className="flex items-center justify-between text-neutral-400">
                    <span>Target: <strong className="text-neutral-200">{post.externalSideEffect.target || "N/A"}</strong></span>
                    <span>Provider: <strong className="text-amber-300">{post.externalSideEffect.service || (post.externalSideEffect as any).provider || "API"}</strong></span>
                  </div>
                  {post.externalSideEffect.statusText && (
                    <div className="text-neutral-400">
                      Status: <span className="text-neutral-200">{post.externalSideEffect.statusText}</span>
                    </div>
                  )}
                  {post.externalSideEffect.evidenceHash && (
