            ...
            {attachments.length > 0 && (
              <div className="flex items-center gap-2 flex-wrap mt-2 p-2 bg-neutral-900/60 rounded-xl border border-neutral-800">
                ...
                {attachments.map((att) => (
                  <div
                    key={att.id}
                    className="inline-flex items-center gap-1.5 px-2 py-1 rounded-lg bg-neutral-950 border border-neutral-700/80 text-neutral-200 text-xs shadow-xs"
                  >
                    {att.type === "image" && (
                      <div className="w-5 h-5 rounded overflow-hidden bg-black shrink-0 border border-neutral-700">
                        <img src={att.url} alt={att.title || "Thumb"} className="w-full h-full object-cover" />
                      </div>
                    )}
                    {att.type === "document" && <FileText className="w-3.5 h-3.5 text-blue-400 shrink-0" />}
                    {att.type === "video" && <Video className="w-3.5 h-3.5 text-purple-400 shrink-0" />}
                    {att.type === "code_snippet" && <Code2 className="w-3.5 h-3.5 text-cyan-400 shrink-0" />}
                    {att.type === "github_repo" && <FolderGit2 className="w-3.5 h-3.5 text-sky-400 shrink-0" />}
                    {att.type === "url" && <Link className="w-3.5 h-3.5 text-emerald-400 shrink-0" />}
                    <span className="truncate max-w-[160px] font-medium">{att.title || (att.type === "image" ? "Imagem" : att.url)}</span>
                    <button
                      type="button"
