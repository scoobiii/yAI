        ...
        try {
          ...
          if (functionCalls && functionCalls.length > 0) {
            for (const call of functionCalls) {
              ...
              } else if (toolName === "mcpCallTool" || toolName.startsWith("mcp_")) {
                const targetTool = toolName.startsWith("mcp_") ? toolName.replace("mcp_", "") : (args.toolName || "github_get_repo");
                toolResult = await AgentSandbox.mcpCallTool(targetTool, args);
              } else if (toolName === "gcloudAuthStatus") {
                toolResult = await AgentSandbox.gcloudAuthStatus();
              } else if (toolName === "gcloudProjectInfo") {
                toolResult = await AgentSandbox.gcloudProjectInfo(args);
              } else if (toolName === "gcloudListGeminiModels") {
                toolResult = await AgentSandbox.gcloudListGeminiModels();
              } else if (toolName === "gcloudListCloudRunServices") {
                toolResult = await AgentSandbox.gcloudListCloudRunServices(args);
              } else if (toolName === "gcloudListStorageBuckets") {
                toolResult = await AgentSandbox.gcloudListStorageBuckets(args);
              } else if (toolName === "gcloudTestConnection") {
                toolResult = await AgentSandbox.gcloudTestConnection();
              } else {
