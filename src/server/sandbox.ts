...
${executableCode}
  ...
  /**
   * Model Context Protocol (MCP) Unified Execution Gateway
   */
  static async mcpCallTool(toolName: string, args: Record<string, any> = {}): Promise<SandboxToolResult> {
    const res = await MCPService.executeTool(toolName, args);
    return {
  ...
  /**
   * OpenClaw & MCP GitHub Actions (100% Real REST API v3)
   */
  static async githubCreateIssue(params: {
    ...
    const mcpRes = await MCPService.executeTool("github_create_issue", {
      repo,
    ...
    return {
      ...
      success: mcpRes.success || mcpRes.claim === "auth_required",
      data: mcpRes.data,
      logs: mcpRes.logs,
      executionTimeMs: mcpRes.executionTimeMs,
      evidenceHash: mcpRes.evidenceHash,
      sideEffectReceipt: {
        service: "github",
        action: "github.createIssue",
        target: repo,
        status: mcpRes.claim === "auth_required" ? "auth_required" : mcpRes.success ? "success" : "error",
        verified: mcpRes.success,
        evidenceHash: mcpRes.evidenceHash,
        latencyMs: mcpRes.executionTimeMs,
        data: mcpRes.data,
        logs: mcpRes.logs,
        timestamp: mcpRes.timestampUtc,
      },
    };
  }
  ...
  static async githubCreatePR(params: {
    ...
    const repo = params?.repoFullName || (params?.owner && params?.repo ? `${params.owner}/${params.repo}` : "scoobiii/vortex");
    const mcpRes = await MCPService.executeTool("github_create_pull_request", {
      repo,
  ...
  static async githubGetRepo(params: {
    ...
    const repo = params?.repoFullName || (params?.owner && params?.repo ? `${params.owner}/${params.repo}` : "scoobiii/vortex");
    const mcpRes = await MCPService.executeTool("github_get_repo", { repo });
    return {
      toolName: "githubGetRepo",
      success: mcpRes.success,
      data: mcpRes.data,
      logs: mcpRes.logs,
      executionTimeMs: mcpRes.executionTimeMs,
      evidenceHash: mcpRes.evidenceHash,
  ...
  static async githubListIssues(params: {
    ...
    const repo = params?.repoFullName || (params?.owner && params?.repo ? `${params.owner}/${params.repo}` : "scoobiii/vortex");
    const mcpRes = await MCPService.executeTool("github_list_issues", {
      repo,
  ...
  // --- Real Google Cloud (GCloud) MCP Tools ---
  static async gcloudAuthStatus(): Promise<SandboxToolResult> {
    const mcpRes = await MCPService.executeTool("gcloud_auth_status");
    return {
      toolName: "gcloudAuthStatus",
      success: mcpRes.success,
      data: mcpRes.data,
      logs: mcpRes.logs,
      executionTimeMs: mcpRes.executionTimeMs,
  ...
  static async gcloudProjectInfo(params?: { projectId?: string }): Promise<SandboxToolResult> {
    const mcpRes = await MCPService.executeTool("gcloud_project_info", params || {});
    return {
      toolName: "gcloudProjectInfo",
      success: mcpRes.success,
      data: mcpRes.data,
      logs: mcpRes.logs,
      executionTimeMs: mcpRes.executionTimeMs,
  ...
  static async gcloudListGeminiModels(): Promise<SandboxToolResult> {
    const mcpRes = await MCPService.executeTool("gcloud_list_gemini_models");
    return {
      toolName: "gcloudListGeminiModels",
      success: mcpRes.success,
      data: mcpRes.data,
      logs: mcpRes.logs,
      executionTimeMs: mcpRes.executionTimeMs,
  ...
  static async gcloudListCloudRunServices(params?: { projectId?: string; region?: string }): Promise<SandboxToolResult> {
    const mcpRes = await MCPService.executeTool("gcloud_list_cloud_run_services", params || {});
    return {
      toolName: "gcloudListCloudRunServices",
      success: mcpRes.success,
      data: mcpRes.data,
      logs: mcpRes.logs,
      executionTimeMs: mcpRes.executionTimeMs,
  ...
  static async gcloudListStorageBuckets(params?: { projectId?: string }): Promise<SandboxToolResult> {
    const mcpRes = await MCPService.executeTool("gcloud_list_storage_buckets", params || {});
    return {
      toolName: "gcloudListStorageBuckets",
      success: mcpRes.success,
      data: mcpRes.data,
      logs: mcpRes.logs,
      executionTimeMs: mcpRes.executionTimeMs,
  ...
  static async gcloudTestConnection(): Promise<SandboxToolResult> {
    const mcpRes = await MCPService.executeTool("gcloud_test_connection");
    return {
      toolName: "gcloudTestConnection",
      success: mcpRes.success,
      data: mcpRes.data,
      logs: mcpRes.logs,
      executionTimeMs: mcpRes.executionTimeMs,
